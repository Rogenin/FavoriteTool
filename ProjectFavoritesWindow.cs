...
public class ProjectFavoritesWindow : EditorWindow
{
    ...
    private const string ColorToggleText = "Color";
    private const string ResetColorButtonText = "Reset";
    ...
    private const float ItemContentSpacing = 4f;
    private const float MissingRowRemoveButtonWidth = 90f;
    private const float MoveButtonWidth = 20f;
    private const float ItemLeftPadding = 10f;
    private const float ItemColorBarWidth = 6f;
    private const float ItemBackgroundInset = 1f;
    private const float ColorToggleWidth = 56f;
    private const float ColorFieldWidth = 70f;
    private const float ResetColorButtonWidth = 52f;
    ...
    private const float ItemBackgroundColorAlpha = 0.14f;
    private static readonly Color DefaultFavoriteColor = new Color(0.25f, 0.60f, 1.00f, 1.00f);
    ...
    private class FavoriteItem
    {
        ...
        public Color color = DefaultFavoriteColor;
    }
    ...
    private void HandleListActions(
        int removeIndex,
        int moveUpIndex,
        int moveDownIndex)
    {
        ...
        ref bool visualSettingsChanged)
    {
        ...
        using (new EditorGUILayout.HorizontalScope())
        {
            ...
            if (GUILayout.Button(SelectButtonText))
            {
            ...
            DrawColorControls(item, ref visualSettingsChanged);
    ...
    private void DrawColorControls(FavoriteItem item, ref bool visualSettingsChanged)
    {
        EditorGUI.BeginChangeCheck();

        bool newUseCustomColor = EditorGUILayout.ToggleLeft(
            ColorToggleText,
            item.useCustomColor,
            GUILayout.Width(ColorToggleWidth));

        Color newColor;
        using (new EditorGUI.DisabledScope(!newUseCustomColor))
        {
            newColor = EditorGUILayout.ColorField(
                GUIContent.none,
                item.color,
        ...
        {
            item.useCustomColor = newUseCustomColor;
            item.color = NormalizeFavoriteColor(newColor);
            visualSettingsChanged = true;
        }

        using (new EditorGUI.DisabledScope(!item.useCustomColor))
        {
            if (GUILayout.Button(ResetColorButtonText, GUILayout.Width(ResetColorButtonWidth)))
            {
                item.color = DefaultFavoriteColor;
                visualSettingsChanged = true;
            }
        }
    }
